...
let snapIndex = 0;
let allSnaps = document.querySelectorAll(".zoomerSlide");

function showSnap(idx) {
  allSnaps.forEach((s, i) => {
    s.classList.remove("showy");
    if (i === idx) s.classList.add("showy");
  });
}

function mover(step) {
  snapIndex = (snapIndex + step + allSnaps.length) % allSnaps.length;
  showSnap(snapIndex);
}
